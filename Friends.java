...
//		boolean success = false; // success becomes true when target is reached
//		Queue<Integer> queue = new Queue<>();
//		boolean[] visitedArray = new boolean[g.members.length];
//		int[] pred = new int[g.members.length];
//
//		Person person1 = g.members[g.map.get(p1)];
//		Person person2 = g.members[g.map.get(p2)];
//
//		for (int v = 0; v < visitedArray.length; v++) {
//			if (!visitedArray[v]) {
//				System.out.println("\nSTARTING AT " + g.members[v].name);
//				queue.clear();
//				bfs(g.members, visitedArray, queue, g);
//			}
//		}
//
...
//	}
//
//	private static void bfs(int start, boolean[] visited, Queue<Integer> queue, Graph g) {
//		visited[start] = true;
//		System.out.println("visiting " + g.members[start].name);
//		queue.enqueue(start);
//
//		while (!queue.isEmpty()) {
//			int v = queue.dequeue();
...
//		Stack<String> resultPath = new Stack<>();
...
//			Scanner sc1 = new Scanner(new File("names2.txt"));
//			Graph graph = new Graph(sc1);
//			// graph.print();
//			System.out.println("\n\n");
//			shortestChain(graph, "michelle", "harita");
//			cliques(graph, "rutgers");
//			cliques(graph, "penn state");
//			cliques(graph, "rowan");
//			cliques(graph, "ucla");
//			cliques(graph, "columbia");
//			connectors(graph);
//			sc.close();
//			sc1.close();
//		} catch (Exception e2) {
